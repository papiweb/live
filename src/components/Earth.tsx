...
function Earth() {
  ...
  useFrame(() => {
    ...
    if (clockHandsRef.current) {
      clockHandsRef.current.children[0].rotation.z = -time.hours * (Math.PI / 6);
      clockHandsRef.current.children[1].rotation.z = -time.minutes * (Math.PI / 30);
      clockHandsRef.current.children[2].rotation.z = -time.seconds * (Math.PI / 30);
    }
  });
